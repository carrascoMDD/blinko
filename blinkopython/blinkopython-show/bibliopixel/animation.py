...
class StripChannelTest_ACV02(BaseStripAnim):

    def __init__(self, led):
        super(StripChannelTest_ACV02, self).__init__(led)
        self._internalDelay = 0
        self.colors = [colors.Red, colors.Green, colors.Blue, colors.White]

    def step(self, amt=1):

        someRainbowColors = colors.hue_rainbow

        for aPixIdx in range( 0, 144):

            aColorIndex = ( aPixIdx + self._step) % 255
            aColor = someRainbowColors[ aColorIndex]
            self._led.set(143 - aPixIdx, aColor)


        self._step += 1
        

class StripChannelTest_ACV03(BaseStripAnim):

    def __init__(self, led):
        super(StripChannelTest_ACV03, self).__init__(led)
        self._internalDelay = 20
        self.colors = [colors.Red, colors.Green, colors.Blue, colors.White]
        self.lastLuminosity = 0
    ...
    def step(self, amt=1):
        ...
        # luminosity = 255 - ( ( self._step / 256) % 256)
        # luminosity = ( ( self._step * 20) / 256) % 256
        # luminosity = 255;
        luminosity = ( ( self._step + 8) / 256) % 256

        
        for aPixIdx in range( 0, 144):

            aColorIndex = ( aPixIdx + self._step) % 256
            aColor = someRainbowColors[ aColorIndex]
            
            otherColor = ( ( aColor[ 0] * luminosity) / 256, ( aColor[ 1] * luminosity) / 256, ( aColor[ 2] * luminosity) / 256)
            
            # self._led.set(143 - aPixIdx, ( 255, 255, 255))
            # self._led.set(143 - aPixIdx, ( 255, 0, 0))
            self._led.set(143 - aPixIdx, otherColor)
...
class StripChannelTest_ACV_fixed(BaseStripAnim):

    def __init__(self, led, theFixedColor):
        super(StripChannelTest_ACV_fixed, self).__init__(led)
        self._internalDelay = 20
        self.colors = [colors.Red, colors.Green, colors.Blue, colors.White]
        self._v_FixedColor = theFixedColor
        

    def step(self, amt=1):
        
        self._led.fill( self._v_FixedColor, 0, 143)
        
        
        for aPixIdx in range( 0, 144):
            self._led.set( aPixIdx, self._v_FixedColor)
